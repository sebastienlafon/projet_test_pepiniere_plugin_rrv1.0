...
function searchOrders() {
  ...
  jQuery.ajax({
     url: pluginsURL + '/hostel-booking/lib/procedure_commandes.php',
     type: 'post',
...
function updateOrder(e) {
  ...
  for(i = 0; i < cells.length-3; i++) {
    ...
    cells[i].innerHTML = cellVals[i];
        ...
        jQuery.ajax({
           url: pluginsURL + '/hostel-booking/lib/maj_commande.php',
           type: 'post',
           data: {"new_values" : cellVals, "booking_ref" : bookingRef},
           success: function(data) {
...
function confirmDeleteOrder(e, action) {
  var cells = e.parentNode.parentNode.getElementsByTagName("td");  
  var bookingRef = cells[4].innerHTML;
  ...
  if (confirm('Are you sure you want to ' + action + ' this order ?')) {
    ...
    jQuery.ajax({
     ...
     type: 'post',
     data: {"booking_ref" : bookingRef, "action" : action},
     success: function(data) {
...
<tr>
  <th>Name</th><th class="email-head">Email</th><th>Phone</th><th>prix</th><th>Booking ref</th><th class="button-head"></td><th class="button-head"></td>
</tr>
...
$current_orders = $wpdb->get_results("SELECT * FROM wp_RR_orders WHERE year = $current_year AND (month = $current_month OR month = $second_month)", ARRAY_A); 
...
$all_orders = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "RR_orders WHERE pending = 0 ORDER BY id DESC", ARRAY_A);


foreach($all_orders as $row) {
echo "<tr id='r{$row['booking_ref']}'><td class='col1'> ". $row['name'] .  "<td class='col2'>" . $row['email'] . "</td><td class='col1'> ". $row['phone'] . "</td><td class='col2'>" . $row['prix'] . "</td><td class='col1'>" . $row['booking_ref'] . "</td>"; 
...
$pending_orders = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "RR_orders WHERE pending = 1", ARRAY_A );

foreach($pending_orders as $row) {
echo "<tr id='o{$row['booking_ref']}'><td class='col1'> ". $row['name'] .  "<td class='col2'>" . $row['email'] . "</td><td class='col1'> ". $row['phone'] . "</td><td class='col2'>" . $row['prix'] . "</td><td class='col1'>" . $row['booking_ref'] . "</td>"; 
